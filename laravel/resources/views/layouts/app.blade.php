...
<body>
    <nav class="navbar">
        <div class="container">
            <a href="{{ route('books.index') }}" class="brand">📚 書籍管理系統</a>
            <div class="nav-links">
                @auth
                    <a href="{{ route('books.index') }}">書籍列表</a>
                    @if(auth()->user()->isSuperAdmin())
                        <a href="{{ route('publishers.index') }}">出版社管理</a>
                    @endif
                    <span class="text-sm">{{ auth()->user()->display_name }}</span>
